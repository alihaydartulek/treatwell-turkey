            ...
            <div className="lg:col-span-2 flex flex-col gap-12">
              ...
              <section>
                ...
                {clinic.reviews.length > 0 ? (
                  ...
                  <div className="text-center py-10 px-6 border border-slate-200 rounded-2xl bg-slate-50">
                    ...
                    <p className="text-sm font-medium text-slate-700 mb-1">
                      ...
                      verified reviews on {clinic.googleReviewCount ? "Google" : (clinic.ratingSource ?? "independent platforms")}
                    </p>
                    <p className="text-xs text-slate-400 mb-5 max-w-sm mx-auto">
                      We link to the clinic&apos;s real reviews rather than copy them here, so you always read the genuine, up-to-date feedback.
                    </p>
                    <a
                      href={`https://www.google.com/search?q=${encodeURIComponent(clinic.name + " " + clinic.city + " reviews")}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-300 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 transition-colors text-sm"
                    >
                      Read all {(clinic.googleReviewCount ?? clinic.reviewCount).toLocaleString()} reviews →
                    </a>
                  </div>
                )}
              </section>
